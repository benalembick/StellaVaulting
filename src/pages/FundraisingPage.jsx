  ...
  return (
    <div className="pt-20">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-14 text-center">
        ...
        <h1 className="font-serif text-5xl md:text-6xl text-brand-ink font-light">Fundraising</h1>
        <p className="mt-6 text-brand-ink-soft max-w-2xl mx-auto leading-relaxed text-lg font-light">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pb-24">
        ...
        {/* Social promise */}
        <div className="mt-20 pink-panel p-10 text-center">
          <div className="text-4xl mb-4">❤️</div>
          <h2 className="font-serif text-2xl text-brand-ink mb-3">Our Promise to You</h2>
          <p className="text-brand-ink-soft max-w-xl mx-auto leading-relaxed">
        ...
        {/* How it works */}
        <div className="mt-20">
          <SectionHeading label="Simple Steps" title="How It Works" center className="mb-12" />
          <div className="grid md:grid-cols-3 gap-8">
            {[
              ...
              <div key={step} className="card-premium p-8">
                <div className="text-5xl font-serif text-brand-gold/30 mb-3">{step}</div>
  ...
  return (
    <div className="card-premium group flex flex-col">
      {product.image ? (
        ...
        <div className="aspect-square bg-brand-surface-warm flex items-center justify-center border-b border-brand-gold/[0.1]">
          <span className="text-6xl">{product.emoji || '🎁'}</span>
        </div>
      )}
      <div className="p-5 flex flex-col flex-1">
        <h3 className="font-serif text-lg text-brand-ink leading-tight">{product.name}</h3>
        {product.description && (
          <p className="text-xs text-brand-ink-soft mt-2 leading-relaxed flex-1">{product.description}</p>
        )}
        <div className="flex items-center justify-between mt-4 pt-4 border-t border-brand-gold/[0.1]">
          <span className="font-serif text-2xl text-brand-gold">${product.price.toFixed(0)}</span>
          <button
            onClick={onAdd}
            className={`flex items-center gap-1.5 px-4 py-2 text-xs font-semibold tracking-wide uppercase rounded-full transition-all duration-200 ${
              justAdded
                ? 'bg-green-50 text-green-600 border border-green-200'
                : 'bg-brand-gold/8 text-brand-gold border border-brand-gold/25 hover:bg-brand-gold hover:text-white'
            }`}
          >
