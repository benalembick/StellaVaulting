...
const TIER_COLORS = {
  ...
  gold: 'border-brand-gold/25 bg-brand-blush',
  silver: 'border-brand-gold/15 bg-brand-surface-warm',
  bronze: 'border-brand-ink/10 bg-brand-surface-alt',
}

const TIER_BADGE = {
  platinum: 'text-brand-ink bg-brand-ink/8',
  gold: 'text-brand-gold bg-brand-gold/10',
  silver: 'text-brand-ink-soft bg-brand-ink/6',
  bronze: 'text-brand-ink/60 bg-brand-ink/5',
}
  ...
  return (
    <div className="pt-20">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-14 text-center">
        ...
        <h1 className="font-serif text-5xl md:text-6xl text-brand-ink font-light">Sponsorship</h1>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pb-24">
        ...
        <div className="card-premium p-8 md:p-12 mb-20 bg-brand-surface-alt border-brand-gold/20">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <p className="text-brand-ink-soft mt-6 leading-relaxed">
            ...
            <div className="grid grid-cols-2 gap-5">
              {[
                ...
                <div key={label} className="pink-panel p-5 text-center">
                  <div className="text-2xl mb-1">{icon}</div>
                  <p className="text-xs text-brand-ink-soft tracking-wide">{label}</p>
        ...
        {/* Benefits */}
        <div className="mt-24 border-t border-brand-gold/20 pt-20">
          <SectionHeading
        ...
        <div className="mt-16 text-center">
          <p className="text-brand-ink-soft mb-6 max-w-xl mx-auto">
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        {item.image && (
          <div className="lg:w-64 flex-shrink-0">
            <img src={item.image} alt={item.title} className="w-full h-48 lg:h-full object-cover rounded-xl" />
          </div>
        )}
        <div className="flex-1">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              {item.tier && (
                <span className={`inline-block text-[10px] tracking-[0.2em] uppercase px-3 py-1 rounded-full mb-3 ${TIER_BADGE[item.tier] || ''}`}>
                  {item.tier} Sponsor
                </span>
              )}
              <h3 className="font-serif text-2xl text-brand-ink">{item.title}</h3>
              {item.description && (
                <p className="text-brand-gold text-sm mt-1">{item.description}</p>
              )}
            </div>
            {item.amount && (
              <div className="text-right">
                <p className="text-xs text-brand-ink/40 tracking-wide">Investment</p>
                <p className="font-serif text-2xl text-brand-gold">${item.amount.toLocaleString()}</p>
              </div>
            )}
          </div>
          {item.body && (
            <p className="text-brand-ink-soft mt-4 leading-relaxed text-sm">{item.body}</p>
          )}
          {item.cta_text && (
            <div className="mt-6">
