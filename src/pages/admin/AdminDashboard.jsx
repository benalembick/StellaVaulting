...
export default function AdminDashboard() {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="font-serif text-3xl text-brand-white">Dashboard</h1>
        <p className="text-brand-white/50 text-sm mt-1">Welcome back to the Stella Vaulting Academy admin panel.</p>
      </div>
        ...
        {cards.map(({ label, value, icon: Icon, to, color }) => (
          <Link key={label} to={to} className="admin-card hover:border-brand-gold/40 transition-all group">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-xs text-brand-white/50 tracking-wider uppercase">{label}</p>
            ...
            <div className="mt-4 flex items-center gap-1 text-xs text-brand-white/30 group-hover:text-brand-gold transition-colors">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="admin-card">
          <h2 className="font-serif text-lg text-brand-white mb-4">Quick Actions</h2>
          <div className="space-y-2">
            {quickLinks.map(({ to, label }) => (
              <Link
                key={to}
                to={to}
                className="flex items-center justify-between px-4 py-2.5 rounded bg-brand-black border border-brand-gold/10 hover:border-brand-gold/30 text-sm text-brand-white/70 hover:text-brand-white transition-all group"
              >
                {label}
        ...
        <div className="admin-card">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-serif text-lg text-brand-white">Recent Orders</h2>
            <Link to="/admin/orders" className="text-xs text-brand-gold hover:text-brand-gold-light transition-colors">
              View all →
            </Link>
          </div>
          {recentOrders.length === 0 ? (
            <div className="text-center py-8 text-brand-white/30">
            ...
            <div className="space-y-3">
              {recentOrders.map((order) => (
                <div key={order.id} className="flex items-center justify-between p-3 bg-brand-black rounded border border-brand-gold/10">
                  <div>
                    <p className="text-sm text-brand-white">{order.customer_name || order.customer_email || 'Anonymous'}</p>
                    <p className="text-xs text-brand-white/40 mt-0.5">{new Date(order.created_at).toLocaleDateString()}</p>
                  </div>
