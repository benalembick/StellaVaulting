  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-serif text-3xl text-brand-white">Events</h1>
          <p className="text-brand-white/50 text-sm mt-1">Manage upcoming and past events</p>
        </div>
      ...
      {editing && (
        <div className="admin-card mb-6">
          <h2 className="font-serif text-xl text-brand-white mb-5">{editing === 'new' ? 'Add Event' : 'Edit Event'}</h2>
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div className="flex items-center gap-6 pt-4">
              <div className="flex items-center gap-2">
                ...
                <button type="button" onClick={() => setForm({ ...form, show_countdown: !form.show_countdown })} className={`w-10 h-5 rounded-full transition-colors ${form.show_countdown ? 'bg-brand-gold' : 'bg-brand-white/20'}`}>
          ...
          {events.map((e) => (
            <div key={e.id} className="admin-card flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-brand-white font-medium truncate">{e.name}</p>
                <div className="flex gap-3 text-xs text-brand-white/40 mt-0.5">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <button onClick={() => toggle(e)} className={e.published ? 'text-green-400' : 'text-brand-white/30'}>{e.published ? <Eye size={14} /> : <EyeOff size={14} />}</button>
                <button onClick={() => openEdit(e)} className="text-brand-white/50 hover:text-brand-gold transition-colors"><Edit2 size={14} /></button>
                <button onClick={() => del(e.id)} className="text-brand-white/30 hover:text-red-400 transition-colors"><Trash2 size={14} /></button>
              </div>
            </div>
          ))}
          {events.length === 0 && <div className="text-center py-12 text-brand-white/30"><p>No events yet.</p></div>}
        </div>
      )}
    </div>
  )
