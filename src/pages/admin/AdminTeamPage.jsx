  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-serif text-3xl text-brand-white">Team Members</h1>
          <p className="text-brand-white/50 text-sm mt-1">Manage athlete and coach profiles</p>
        </div>
      ...
      {editing && (
        <div className="admin-card mb-6">
          <h2 className="font-serif text-xl text-brand-white mb-5">
          ...
          <div className="grid md:grid-cols-2 gap-5">
            ...
            <div className="flex items-center gap-3 pt-5">
              ...
              <button
                ...
                className={`w-10 h-5 rounded-full transition-colors ${form.published ? 'bg-brand-gold' : 'bg-brand-white/20'}`}
              >
        ...
        <div className="space-y-3">
          {members.map((m) => (
            <div key={m.id} className="admin-card flex items-center gap-4">
              <GripVertical size={16} className="text-brand-white/20 flex-shrink-0" />
              ...
              <div className="flex-1 min-w-0">
                <p className="text-brand-white font-medium truncate">{m.name}</p>
                {m.role && <p className="text-brand-pink text-xs mt-0.5">{m.role}</p>}
                <p className="text-brand-white/40 text-xs mt-0.5 truncate">{m.bio}</p>
              </div>
              <div className="flex items-center gap-2 flex-shrink-0">
                <button onClick={() => togglePublish(m)} className={`text-xs flex items-center gap-1 ${m.published ? 'text-green-400' : 'text-brand-white/30'}`}>
                  {m.published ? <Eye size={14} /> : <EyeOff size={14} />}
                </button>
                <button onClick={() => openEdit(m)} className="text-brand-white/50 hover:text-brand-gold transition-colors"><Edit2 size={14} /></button>
                <button onClick={() => del(m.id)} className="text-brand-white/30 hover:text-red-400 transition-colors"><Trash2 size={14} /></button>
              </div>
            </div>
          ))}
          {members.length === 0 && (
            <div className="text-center py-12 text-brand-white/30">
